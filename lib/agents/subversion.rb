    ...
    class Subversion<RPC::Agent
      ...
      def prop_wrapper (act, ctx, target)
        ...
        when "propget" 
          ...
          values = []
           begin
            ctx.propget(actual_property, target, revision, nil, recurse) do |path, val|
              files.push(path)
              values.push(val)
            end
          rescue Exception => err
        ...
        when "proplist"
          begin
            ...
            ctx.proplist(target) do |item,val|
              item.sub!(url, target)
              val.each do |key, var|
                reply[:output] = "#{key} => #{propindex[key]} "
                properties[propindex[key]][item] = var
              end
            end
            for i in 0..5 
